 ...
 public:
  ...
  class Value {
   public:
    ...
    explicit Value(Json::Value& document, const ValueType type)
    {
    ...
    STATUSTYPE Swap(Json::Value& other)
    {
    ...
    STATUSTYPE Add(const char* name, Json::Value&& value)
    {
      rapidjson::Value& object = AsMutableValue();
      if (!object.IsObject()) {
        STATUSRETURN(
    ...
    STATUSTYPE Append(Json::Value&& value)
    {
    ...
    // error if not.
    STATUSTYPE AssertType(Json::ValueType type) const
    {
      if (static_cast<rapidjson::Type>(type) != AsValue().GetType()) {
        STATUSRETURN(std::string("unexpected type"));
      }
      return STATUSSUCCESS;
    }
    ...
    STATUSTYPE At(
        const size_t idx, Json::Value* value = nullptr)
    {
      rapidjson::Value& array = AsMutableValue();
      ...
      *value = Json::Value(array[idx], allocator_);
      return STATUSSUCCESS;
    }
    ...
    bool Find(const char* name, Json::Value* value)
    {
      rapidjson::Value& object = AsMutableValue();
      if (object.IsObject() && object.HasMember(name)) {
        if (value != nullptr) {
          *value = Json::Value(object[name], allocator_);
        }
        return true;
      }
    ...
    STATUSTYPE MemberAsArray(
        const char* name, Json::Value* value)
    {
      rapidjson::Value& object = AsMutableValue();
      if (!object.IsObject() || !object.HasMember(name)) {
        STATUSRETURN(
      ...
      *value = Json::Value(v, allocator_);
      return STATUSSUCCESS;
    }

    // Get named object member contained in this object.
    STATUSTYPE MemberAsObject(
        const char* name, Json::Value* value)
    {
      rapidjson::Value& object = AsMutableValue();
      if (!object.IsObject() || !object.HasMember(name)) {
    ...
    STATUSTYPE IndexAsArray(
        const size_t idx, Json::Value* value)
    {
      rapidjson::Value& array = AsMutableValue();
      if (!array.IsArray() || (idx >= array.GetArray().Size())) {
        STATUSRETURN(
